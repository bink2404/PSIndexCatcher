...
HICON	winIcon ;
HWND	hAboutBox, hFindWindow = NULL, hReplaceWindow, hIgnoreWindow = NULL, hButtonWindow = NULL, hSeparatorWindow = NULL, hStats = NULL ;
...
BOOL	APIENTRY StatsProc(HWND, UINT, WPARAM, LPARAM) ;
...
//			_beginthread(startThreads, 16384, (PVOID)NULL) ;
		...
		case WM_CTLCOLORLISTBOX:
			...
			// Make sure that the previous background brush is deleted to avoid a memory leak
			DeleteObject(bkBrush) ;
			bkBrush = CreateBrushIndirect(&LogBrush) ;

			return((LPARAM)bkBrush);
...
//					sendDDEMessage(dlg, ITEMFINDPAGE, itemList[selected].itemPage[0]) ;
			...
			switch (wParam)
			{
				...
				case ID_DISPLAYSTATS :
					...
					DialogBox(appInstance, MAKEINTRESOURCE(IDD_STATS), dlg, (DLGPROC)StatsProc) ; 
				...
				case ID_PRINTMARKERS :
                    ...
                    if (PrintDlg(&pd)) 
                    { 
                        SendMessage(GetDesktopWindow(), WM_PRINTCLIENT, (WPARAM) pd.hDC, PRF_ERASEBKGND|PRF_CLIENT);
...
/*	CHAR alphChar, compareChar ;

	for (int iC = iCount ; iC >= 0 ; iC--)
	{
		if (szAddString[iC][0] != ' ')
		{
			alphChar = toupper(szAddString[iC][0]) ;
			compareChar = toupper(szAddString[iC-1][0]) ;

			//aardvark
			if ((alphChar != compareChar) && (alphChar != ' ') && (compareChar != ' '))
			{
				for (int st = iCount ; st >= iC ; st--)
				{
					strcpy(szAddString[st+1], szAddString[st]) ;
				}

				szAddString[iC] [0] = alphChar ;
				szAddString[iC] [1] = '\0' ;
			}
		}
	} */
...
{
//	INT		inCount ;
	char	szTempString1[MAXLEN], szTempString2[MAXLEN], szTempString3[MAXLEN], szTempString4[MAXLEN], szTempString5[MAXLEN] ;
	char	s1[MAXLEN], s2[MAXLEN], s3[MAXLEN], s4[MAXLEN] ;
	char	szAddTempTabbed[MAXLEN], szCompareTempTabbed[MAXLEN] ;
...
/*		for (k = 0 ; k < (INT)strlen(inItemList[inCount].itemText) ; k++)
		...
		// Check for ':'
		for (iSubString = 1 ; iSubString < (int)strlen(szinAdd) ; iSubString++)
...
/*		if (szCompareSearchString[0] == 'a')
		...
		for (i = 0 ; i < iCount ; i++)
		{
			...
			getCompareText(szCompareSearchString, szCompare2) ;

			// Sort override entries will include any colon's, so strip them out
			for (int ps = 0 ; ps < (int)strlen(szCompare1) ; ps++)
			{
				if ((szCompare1[ps] == ':') && (isEscapeSequence(szCompare1, ps) == FALSE))
				{
					szCompare1[ps] = '\0' ;
					break ;
				}
				...
				else
				{
					for (iTabbed = 0 ; iTabbed < 5 ; iTabbed++)
					{
						...
						for (j = i+1 ; j < iCount ; j++) 
						{
							...
							stripSpaces(szAddTempTabbed) ;
							stripSpaces(szTempString2) ;
							stripSpaces(szTempString3) ;

							for (l = 0 ; l < (int)strlen(szCompare2) ; l++)
							{
								if ((szCompare2[l] == ':') && (isEscapeSequence(szCompare2, l) == FALSE))
								{
									szCompare2[l] = '\0' ;
									break ;
								}
							...
							getAddText(szTempString2, szTempString5) ;

							if ((strncmp(strupr(szCompare1), strupr(szCompare2), strlen(szCompare2)) == 0) &&
								(strlen(szCompare1) == strlen(szCompare2)) &&
								((noTagCompare(szAddTempTabbed, szTempString3) == 0) ||
								 (noTagCompare(szAddTempTabbed, szTempString2) == 0) ||
								 (noTagCompare(szAddTempTabbed, szTempString5) == 0)))
							{
								// Updated tabbed entry.
								getFullItemText(j, s1) ;
								getUnformattedText(s1, s2) ;
								getUnformattedText(inItemList[inCount].itemText, s3) ;

								stripSpaces(s2) ;
								stripSpaces(s3) ;

								s4[0] = '\0' ;
								stripAllSpaces(s2, s4) ;
								strcpy(s2, s4) ;
								s4[0] = '\0' ;
								stripAllSpaces(s3, s4) ;
								strcpy(s3, s4) ;
								...
								for (k = 0 ; k < MAXITEMS ; k++)
								{
									if (((int)inItemList[inCount].itemPage[0] == itemList[j].itemPage[k]) || (szAddTabText[iTabbed+1] [0] != '\0'))
									{
										...
										{
											if ((int)inItemList[inCount].itemPage[0] == itemList[j].itemPage[k])
											{
												// Collapse the range onto the page.
												iRange[j] [k] = -1 ;
											}
										...
										if (((strcmp(inItemList[inCount].itemReference[0], itemList[j].itemReference[k]) == 0) && (strcmp(s2, s3) == 0)) || 
											(szAddTabText[iTabbed+1] [0] != '\0'))
										{
											i = j ;
											break ;
										}
			...
			// Sort override entries will include any colon's, so strip them out
			for (ps = 0 ; ps < (int)strlen(szCompare2) ; ps++)
			{
				if ((szCompare2[ps] == ':') && (isEscapeSequence(szCompare2, ps) == FALSE))
				{
					szCompare2[ps] = '\0' ;
					break ;
				}
			...
			if (((compareSortText(strupr(szCompare1), strupr(szCompare2), strlen(szCompare2)) < 0) &&
				(strncmp(szTempString2, "@>", 2) != 0) && (strchr(szCompare2, ':') == NULL)) ||
				(((strncmp(strupr(szCompare1), strupr(szCompare2), strlen(szCompare2)) == 0) &&
		...
		if ((i == iCount) || (bAddNew == TRUE))
		{
			...
			else
			{
				...
				if ((szAddSearchString[0] == '<') && (isEscapeSequence(szAddSearchString, 0) == FALSE))
				{
					while ((szAddSearchString[st] != '>') && (st < (INT)strlen(szAddSearchString)))
					{
						st++ ;
					}
				...
				// Cope with problem when top-level tag begins with an escape sequence
/*				if (szAddSearchString[st] == '\\')
				{ */
					if (sortOverride == TRUE)
					{
						strcat(itemList[i].itemText, "[") ;
						strcat(itemList[i].itemText, szinCompare) ;
						strcat(itemList[i].itemText, "]") ;
					}
//				}
                        ...
                        WPARAM wParam, LPARAM lParam )
{
	switch( message )
	{
		case WM_INITDIALOG :
			...
			// Set the maximum text allowed in the text boxes
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY1), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY2), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY3), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY4), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY5), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY6), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY7), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY8), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY9), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_DISPLAY10), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION1), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION2), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION3), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION4), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION5), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION6), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION7), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION8), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION9), EM_SETLIMITTEXT, 80, 0) ;
			SendMessage(GetDlgItem(hWnd, IDC_ACTION10), EM_SETLIMITTEXT, 80, 0) ;
...
BOOL APIENTRY StatsProc( HWND hWnd, UINT message, 
                            WPARAM wParam, LPARAM lParam )
{
	CHAR szDisplay[160] ;
	CHAR szTemp[MAXLEN] ;
	...
	LONG lCount ;
	LONG lDepth ;
	LONG lLength, l, totLen, numEntries ;
	LONG tabCount ;
	...
	switch( message )
	{
		case WM_INITDIALOG :
			...
			// Set the stats text
			// Strip off the quotation marks around the file name
			strcpy(szTemp, &g_FileName[1]) ;
			szTemp[strlen(szTemp)-1] = '\0' ;
			SetDlgItemText(hWnd, IDC_TITLE, szTemp) ;
			...
			for (i = 0 ; itemList[i].itemText[0] != '\0' ; i++)
			{
				for (j = 0 ; itemList[i].itemPage[j] != -1 ; j++)
				{
					lCount++ ;
				...
				tabCount = getTabCount(itemList[i].itemText) ;
				...
				if ((itemList[i].itemPage[0] == -1) && (tabCount > 0))
				{
					lCount++ ;
				}
			}

			sprintf(szDisplay, "Number of entries ... %ld", lCount) ;
			SetDlgItemText(hWnd, IDC_STAT1, szDisplay) ;
			...
			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Number of lines ... %ld", lCount) ;
			SetDlgItemText(hWnd, IDC_STAT2, szDisplay) ;

			// Find the number of cross-references
			lCount = 0 ;

			for (i = 0 ; itemList[i].itemText[0] != '\0' ; i++)
			{
				strcpy(szTemp, itemList[i].itemText) ;
				strupr(szTemp) ;

				if ((strnicmp(szTemp, "see also", 8) == 0) ||
					(strnicmp(szTemp, "see", 3) == 0) ||
					((strstr(szTemp, "SEE ALSO")) != NULL) ||
					((strstr(szTemp, "SEE")) != NULL))
				{
					lCount++ ;
				}
			}
			...
			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Number of cross-references ... %ld", lCount) ;
			SetDlgItemText(hWnd, IDC_STAT3, szDisplay) ;

			// Find the number of page ranges
			lCount = 0 ;

			for (i = 0 ; i < (MAXITEMS+1) ; i++)
			{
				for (j = 0 ; j < (INT)strlen(szPageString[i]) ; j++)
				{
					if (strnicmp(&szPageString[i] [j], szSeparatorChars, strlen(szSeparatorChars)) == 0)
					{
						lCount++ ;
					}
				}

/*				if (strstr(szPageString[i], szSeparatorChars) != NULL)
				{
					lCount++ ;
				} */
			}
			...
			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Number of page ranges ... %ld", lCount) ;
			SetDlgItemText(hWnd, IDC_STAT4, szDisplay) ;

			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Deepest tab ... %ld", lDepth) ;
			SetDlgItemText(hWnd, IDC_STAT5, szDisplay) ;

			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Longest entry ... %ld chars", lLength) ;
			SetDlgItemText(hWnd, IDC_STAT6, szDisplay) ;

			szDisplay[0] = '\0' ;
			sprintf(szDisplay, "Average entry ... %ld chars", (totLen / numEntries)) ;
			SetDlgItemText(hWnd, IDC_STAT7, szDisplay) ;
...
BOOL getCompareText(CHAR *inText, CHAR *outText)
{
	...
	for (i = 0, j = 0 ; i < (INT)strlen(inText) ; i++)
	{
		if ((inText[i] == '<') && (isEscapeSequence(inText, i) == FALSE)/* && (inText[i+1] == '$')*/)
		{
			while ((inText[i] != '>') && (i < (INT)strlen(inText)))
			{
				i++ ;
	...
	if ((outText[strlen(outText)-1] == ';') && (isEscapeSequence(outText, strlen(outText)-1) == FALSE))
...
/*	removeTagText(inText1, (CHAR *)s1) ;
	...
	getCompareText(inText1, (CHAR *)s1) ;
	getCompareText(inText1, (CHAR *)s2) ;
...
{
	FILE *ff ;
	char szTempString[MAXLEN] ;

	ff = fopen("c:\\temp.txt", "w+") ;

	int size = SendMessage(LBox, LB_GETCOUNT, 0, 0) ;

	fwrite("\027\069", 2, 1, ff) ;

	for (int i = 0 ; i < size ; i++)
	{
		szTempString[0] = '\0' ;
		SendMessage(LBox, LB_GETTEXT, i, (LPARAM)szTempString) ;
		fwrite(szTempString, strlen(szTempString), 1, ff) ;
		fwrite("\n", 1, 1, ff) ;
	}
