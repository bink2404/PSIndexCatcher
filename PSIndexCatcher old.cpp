...
HICON	winIcon ;
HWND	hAboutBox, hFindWindow = NULL, hReplaceWindow, hIgnoreWindow = NULL, hSeparatorWindow = NULL ;
BOOL	bAboutBoxDisplayed ;
HFONT	defaultFont, italicFont, boldFont, bolditalicFont ;
BOOL	g_bOrphanAudio, g_bOrphanVisual ;
BOOL	g_bRealTimeUpdates, g_bAlwaysOnTop ;
INT		g_iSortOption ;
HWND	LBox ;
INT		fontHeight ;
...
UINT MSG_TASKBARCALLBACK = ::RegisterWindowMessage("TaskbarMessage");
...
BOOL CALLBACK emDEXProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	RECT	pRect ;
	CHAR	szTemp[MAXLEN] ;
	char	szAtom[MAXLEN], szNumString[80], szTempString1[MAXLEN], szTempString2[MAXLEN] ;
...
//			SendMessage(dlg, WM_TIMER, 0, 0);
			...
			if (strlen(g_FileName) == 0)
			{
				...
				tnib.uCallbackMessage    = MSG_TASKBARCALLBACK;
...
/*			if (lpDIS->itemID > itemLimit)
			...
			hDeffont = (HFONT)SelectObject(lpDIS->hDC, defaultFont) ;
			...
			for (i = 0 ; i < (INT)strlen(szAddString[lpDIS->itemID]) ; i++)
			{
				while ((szAddString[lpDIS->itemID] [i] == '<') && (szBracketsDisplayed[lpDIS->itemID] [i] == 0))
				{
					if (strncmp(&szAddString[lpDIS->itemID] [i], "<Bold>", 6) == 0)
					{
						...
						{
							(HFONT)SelectObject(lpDIS->hDC, bolditalicFont) ;
							i += 6 ;
							currentFormat = BOLDITALIC ;
						}
						else
						{
							(HFONT)SelectObject(lpDIS->hDC, boldFont) ;
							i += 6 ;
							currentFormat = BOLD ;
						}
					}
					else
					{
						if (strncmp(&szAddString[lpDIS->itemID] [i], "<Emphasis>", 10) == 0)
						{
							...
							{
								(HFONT)SelectObject(lpDIS->hDC, italicFont) ;
								i += 10 ;
						...
						else
						{
							...
							{
								(HFONT)SelectObject(lpDIS->hDC, defaultFont) ;
								i += 19 ;
								currentFormat = DEFAULT ;
							}
							else
...
/*			if (isProcessing == FALSE)
		...
		case ITEMLIST_UPDATE :
			...
			for (i = 0 ; i < (INT)strlen(szAtom) ; i++)
				if (szAtom[i] == '@')
					break ;

			strncpy(inItemList[inCount].itemReference[0], szAtom, i) ;
			inItemList[inCount].itemReference[0][i] = '\0' ;

			strcpy(inItemList[inCount].itemText, &szAtom[i+1]) ;
			inItemList[inCount].itemPage[0] = wParam ;
...
//			_beginthread(startThreads, 16384, (PVOID)NULL) ;
		...
		case CATCHER_OPEN:
			...
			ShellExecute(NULL, "open", executePath, szExecString, NULL, SW_SHOWNORMAL) ;
